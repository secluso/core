...
pub fn run_provision(app: &AppHandle, run_id: Uuid, target: SshTarget, plan: ServerPlan) -> Result<()> {
  ...
  let server_url_for_preflight = plan.secrets.as_ref().map(|secrets| secrets.server_url.as_str());
  let preflight = run_preflight(app, run_id, "preflight", &sess, &target, Some(&plan.runtime), server_url_for_preflight)?;
