...
use crate::provision_server::ssh::{
    cleanup_remote_path, connect_ssh, create_remote_temp_dir, exec_remote_script_streaming,
    scp_upload_bytes, sudo_prefix,
};
...
use secluso_update::{
    build_github_client, download_and_verify_component, fetch_latest_release,
    Component as ReleaseComponent,
};
use std::fs;
...
const INSTALL_BIN_DIR: &str = "/usr/bin";
const VERSION_ROOT: &str = "/var/lib/secluso/current_version";
...
fn download_verified_artifacts(
    ...
    log_line(
        ...
        Some("artifacts"),
        format!(
            "Latest immutable release for {owner_repo}: {}",
            release.tag_name
        ),
    );
    ...
    let bundle_dir =
        shared_temp_dir("secluso-server-bundle").context("creating temp bundle dir")?;
...
pub fn run_provision(
    ...
    log_line(
        ...
        "info",
        Some("detect"),
        format!("REMOTE_HAS_BIN={remote_has_bin}"),
    );
    log_line(
        app,
        run_id,
        "info",
        Some("detect"),
        format!("REMOTE_HAS_UNIT={remote_has_unit}"),
    );
    log_line(
        app,
        run_id,
        "info",
        Some("detect"),
        format!("REMOTE_SERVICE_ACTIVE={}", preflight.service_active),
    );
    log_line(
        app,
        run_id,
    ...
    log_line(
        ...
        Some("detect"),
        format!(
            "REMOTE_PORT_{}_IN_USE={}",
            plan.runtime.listen_port, preflight.port_in_use
        ),
    );
    ...
    let provision_result = (|| -> Result<()> {
        step_start(
            ...
            run_id,
            "artifacts",
            "Downloading verified release binaries",
        );
        ...
        if first_install {
            let work_dir =
                shared_temp_dir("secluso-server-creds").context("creating temp work dir")?;
            ...
            let uc = std::fs::read(&uc_path)
                .with_context(|| format!("Missing user credentials at {}", uc_path.display()))?;
            let credentials_full_path = work_path.join("credentials_full");
            let credentials_full = std::fs::read(&credentials_full_path).with_context(|| {
                format!(
                    "Missing credentials_full at {}",
                    credentials_full_path.display()
                )
            })?;
            ...
            std::fs::copy(&qr_src, &qr_path)
                .with_context(|| format!("Saving QR code to {}", qr_path.display()))?;
        ...
        let mut envs = vec![
            ("INSTALL_BIN_DIR", INSTALL_BIN_DIR.to_string()),
...
fn verify_public_server_health(
    ...
    let (username, password) = parse_user_credentials(user_credentials.to_vec())
        .context("Parsing generated user credentials")?;
    ...
    if let Some(server_version) = discovered_version {
        log_line(
            ...
            "info",
            Some("health"),
            format!("Remote server version header: {server_version}"),
        );
    }
...
fn unreachable_public_status_error(
    exposure_mode: &str,
    listen_port: u16,
    err: &reqwest::Error,
) -> anyhow::Error {
