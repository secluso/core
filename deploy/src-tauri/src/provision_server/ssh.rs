...
pub fn read_host_key_proof(sess: &Session) -> Result<HostKeyProof> {
    ...
    let (host_key, host_key_type) = sess
        .host_key()
...
pub fn connect_ssh(target: &SshTarget) -> Result<(Session, TempKeyFiles)> {
    ...
    if expected_host_key.sha256 != presented_host_key.sha256
        || expected_host_key.algorithm != presented_host_key.algorithm
    {
        bail!(
    ...
    match &target.auth {
        ...
        SshAuth::KeyText { text, passphrase } => {
            ...
            f.write_all(text.as_bytes())
                .context("Failed writing temp key")?;
            f.flush().ok();
        ...
        "same" => {
            ...
            match &target.auth {
                SshAuth::Password { password } if !password.is_empty() => {
                    ("sudo -S -p ''".to_string(), Some(password.clone()))
                }
...
pub fn exec_remote_script_streaming(
    ...
    channel
        .exec(&cmd)
        .with_context(|| format!("Failed to exec remote bash: {cmd}"))?;
...
pub fn scp_upload_bytes(sess: &Session, remote_path: &str, mode: i32, bytes: &[u8]) -> Result<()> {
    ...
    remote
        .write_all(bytes)
        .context("Failed to write SCP bytes")?;
    remote.send_eof().ok();
    remote.wait_eof().ok();
    remote.close().ok();
