...
pub fn run_preflight(
    ...
    log_line(
        ...
        "info",
        Some(step),
        "Starting server preflight checks.",
    );
    ...
    log_line(
        ...
        "info",
        Some(step),
        format!("Remote OS kernel: {kernel}"),
    );

    let os_release = remote_shell(
        sess,
    ...
    if let Some(name) = pretty_name {
        log_line(
            ...
            "info",
            Some(step),
            format!("Remote distribution: {name}"),
        );
    }
    ...
    if !remote_success(sess, "command -v systemctl >/dev/null 2>&1", None)? {
        bail!(
            "systemctl is missing. Secluso provisioning expects systemd utilities to be installed."
        );
    }
    ...
    let service_active = remote_success(
        sess,
        "systemctl is-active --quiet secluso-server.service",
        None,
    )?;
    ...
    if remote_has_bin || remote_has_unit {
        ...
        log_line(
            ...
            "info",
            Some(step),
            "No existing Secluso server install detected.".to_string(),
        );
    }

    let listen_port = runtime
        .map(|value| value.listen_port)
        .unwrap_or(DEFAULT_SERVER_HTTP_PORT);
    let removed_stale_probe =
        cleanup_stale_preflight_port_listener(app, run_id, step, sess, target, listen_port)?;
    ...
    let occupied_by_secluso = port_probe.stdout.contains("secluso-server")
    ...
    if port_in_use {
        for line in &port_lines {
            log_line(
                ...
                "warn",
                Some(step),
                format!("Port {listen_port} listener: {line}"),
            );
        }
      ...
      format!("Port {listen_port} is already in use by an existing Secluso install or compatible listener."),
        ...
        log_line(
            ...
            "info",
            Some(step),
            format!("Port {listen_port} is free on the server."),
        );
    }
...
fn verify_sudo_access(
    ...
    if target.user == "root" {
        log_line(
            ...
            "info",
            Some(step),
            "SSH user is root; sudo check not needed.".to_string(),
        );
        return Ok(());
    }
    ...
    log_line(
        ...
        "info",
        Some(step),
        format!("Verified sudo access using {mode_label}."),
    );
    Ok(())
}
...
fn verify_public_http_reachability(
    ...
    let exposure_mode = runtime
        .map(|value| value.exposure_mode.as_str())
        .unwrap_or("direct");
    let listen_port = runtime
        .map(|value| value.listen_port)
        .unwrap_or(DEFAULT_SERVER_HTTP_PORT);
    let bind_address = runtime
        .map(|value| value.bind_address.as_str())
        .unwrap_or("0.0.0.0");
    ...
    if exposure_mode != "direct" {
        log_line(
            ...
            "info",
            Some(step),
            "Skipping public port probe during preflight because reverse proxy mode is selected."
                .to_string(),
        );
        return Ok(());
    }

    let Some(public_server_url) = public_server_url
        .map(str::trim)
        ...
        log_line(
            ...
            "warn",
            Some(step),
            "Skipping public port probe during preflight because no public URL was provided."
                .to_string(),
        );
        return Ok(());
    };
...
fn prepare_direct_probe_base_url(public_server_url: &str, listen_port: u16) -> Result<Url> {
    let parsed = Url::parse(public_server_url).with_context(|| {
        format!("Public URL '{public_server_url}' is not a valid URL for direct-mode preflight.")
    })?;
...
printf 'UNIT=%s\\nROOT=%s\\n' '{unit_name}' \"$probe_root\""
    ...
    if result.exit == 126
        && result
            .stderr
            .contains("SECLUSO_HTTP_PROBE_SYSTEMD_RUN_MISSING")
    {
        return Ok(None);
    }
    if result.exit == 124 {
...
rm -rf '{}'",
    ...
    if result.exit != 0 {
        bail!(
            "Temporary HTTP probe cleanup failed. {}",
            summarize_remote_failure(&result)
        );
    }
    Ok(())
...
fn probe_public_demo_endpoint(probe_url: &str, probe_token: &str, listen_port: u16) -> Result<()> {
    ...
    let probe_timeout_budget_secs = (PUBLIC_PROBE_ATTEMPTS as u64
        * PUBLIC_PROBE_HTTP_TIMEOUT.as_secs())
        ...
        match client.get(probe_url).send() {
            Ok(response) => {
                ...
                let body = response.text().with_context(|| {
                    format!("Reading HTTP probe response body from {probe_url}")
                })?;
    ...
    for (host, warning) in dns_checks {
        ...
        if remote_success(sess, &probe, None)? {
            log_line(
                ...
                "info",
                Some(step),
                format!("DNS lookup for {host} succeeded."),
            );
        } else {
            log_line(
                app,
                ...
                "warn",
                Some(step),
                format!("DNS lookup for {host} failed. {warning}"),
            );
        }
    }
      ...
      timeout = REMOTE_HTTPS_PROBE_MAX_TIME_SECS
        ...
        match result.exit {
            0 => log_line(
                ...
                "info",
                Some(step),
                format!("Outbound HTTPS to {url} succeeded."),
            ),
            42 => log_line(
                app,
                run_id,
                ...
                Some(step),
                format!(
                    "curl is not installed yet, so the remote HTTPS probe for {url} was skipped."
                ),
            ),
            _ => log_line(
                app,
                ...
                "warn",
                Some(step),
                format!("Outbound HTTPS probe for {url} failed. {warning}"),
            ),
        }
    }
    ...
    if status.contains("inactive") {
      ...
      format!(
        ...
        log_line(
            ...
            "info",
            Some(step),
            format!("ufw appears to allow TCP port {listen_port}."),
        );
      ...
      format!("ufw is active and does not allow TCP port {listen_port}; adding an allow rule with user permission."),
        ...
        log_line(
            ...
            "info",
            Some(step),
            format!("Added ufw allow rule for TCP port {listen_port}."),
        );
    } else {
        bail!(
...
fn looks_like_stale_preflight_listener(cwd: &str, cmd: &str, listen_port: u16) -> bool {
    if cwd.starts_with("/tmp/secluso-preflight-http.")
        || cmd.contains("/tmp/secluso-preflight-http.")
    {
        return true;
    }
        ...
        "same" => match &target.auth {
            ...
            _ => {
                ...
                if res.exit == 0 {
                    ...
                    remote_shell(sess, fallback_cmd, None)
                }
            }
        },
