//! Utility/shared code for an MP4 Camera
//!
...
/// Mp4Camera uses the shared stream for both motion detection (via raw YUV420 frames) and recording/livestreaming (via H.264).
pub struct Mp4Camera<P> {
    name: String,
    state_dir: String,
    video_dir: String,
...
impl<P> Mp4Camera<P> {
    ...
    async fn write_fmp4(
        ...
        let mut fmp4 = Fmp4Writer::new(
            H264VideoParameters::new(sps.to_vec(), pps.to_vec(), resolution),
            AacAudioParameters::new(),
            livestream_writer,
        )
        .await?;
...
impl CodecParameters for H264VideoParameters {
...
struct AacAudioParameters {
    sample_rate: u32, // 48000
    channels: u16,    // 1
    asc: [u8; 2],     // AudioSpecificConfig
}

impl AacAudioParameters {
    fn new() -> Self {
        Self {
            sample_rate: 48_000,
            channels: 1,
...
impl CodecParameters for AacAudioParameters {
    fn write_codec_box(&self, buf: &mut BytesMut) -> Result<(), Error> {
